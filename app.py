...
def get_pokemon_profile(new_link):
    '''
    get pokedex, name, image source, abilityindex, height, weight, type1, type2 from provided website
...
def get_ability_effect(effect_link):
    ...
    effect = results['effect_entries'][0]['effect']
    return effect
...
def insert_row_to_abilities(value):
    ...
    insert_abilities = '''
        ...
        VALUES (NULL, ?, ?, ?)
    '''
    cur.execute(insert_abilities, value)
    conn.commit()
...
def create_pokemon_table():
    ...
    create_pokemon = '''
        CREATE TABLE IF NOT EXISTS "Pokemon" (
            ...
            'Type1' TEXT NOT NULL,
            'Type2' TEXT,
            FOREIGN KEY(Ability) REFERENCES Abilities(AbilityIndex)
        );
    '''
...
def insert_row_to_pokemon(row):
    ...
    insert_abilities = '''
        ...
        VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?)
    '''
    cur.execute(insert_abilities, row)
    conn.commit()

def find_pokemon_in_db(like_type):
    conn = sqlite3.connect('pokeInfo.sqlite')
    cur = conn.cursor()

    q = f'''
        SELECT Name, ImageSource, AbilityName, Effect, Height, Weight, Type1, Type2
        FROM Pokemon
        JOIN Abilities
            ON Ability=Abilities.AbilityIndex
        WHERE Type1 = '{like_type}' OR Type2 = '{like_type}'
        ORDER BY Pokedex
        LIMIT 1
    '''
...
def profile():
    ...
    for i in range(int(like_number)-10,int(like_number)+11):
        ...
        ability_index = row[3]
        effect_link = baseurl + 'ability/' + ability_index
...
if __name__ == '__main__':
    create_ability_table() #will be deleted every time re-run the app
